...
def get_happy_reviews():
    happy_reviews = Reviewdata[Reviewdata['Is_Response'] == 'happy']['cleaned_description_new'].tolist()
    return jsonify({'happy_reviews': happy_reviews})

@app.route('/api/not_happy_reviews', methods=['GET'])
def get_not_happy_reviews():
    not_happy_reviews = Reviewdata[Reviewdata['Is_Response'] == 'not happy']['cleaned_description_new'].tolist()
...
def get_percentage_not_happy_reviews():
    total_reviews = len(Reviewdata)
    not_happy_reviews_count = len(Reviewdata[Reviewdata['Is_Response'] == 'not happy'])
    percentage_not_happy_reviews = (not_happy_reviews_count / total_reviews) * 100
